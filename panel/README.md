...
- **Eventos recientes (agregado 2026-08-12)**: lista en vivo, en la columna derecha del panel, de lo que va llegando por `.../events` (ver `comms_agent/README.md`) — el mismo stream que alimenta el JSONL del "libro de rondas digital" de `patrol_fsm`. Cada entrada muestra hora + nombre del evento + sus campos extra, más nuevo arriba, tope de 30 en pantalla (no se guarda historial, es solo lo que llegó mientras la pestaña estuvo abierta). Coloreado por severidad (`SEVERIDAD_POR_EVENTO` en `index.html`) — hoy es solo una señal visual, todavía **no** existe la semántica real de alarma-pendiente-hasta-reconocida que motivó agregar este canal (ver diseño completo en la sección de `FALLA` de la memoria `project_external_review_2026_08`); eso queda para una vuelta futura. A diferencia de `state`/`position`/`heartbeat`, `server.py` no lo cachea en `last_known` — es un hecho puntual, un navegador que se conecta después no recibe eventos viejos como si fueran nuevos.
...
- **Conexión inicial no bloqueante (2026-08-13, mismo fix que `comms_agent`, ver `comms_agent/README.md`)**: `make_mqtt_client` usa `connect_async()` en vez de `connect()`. Antes, si Mosquitto no estaba levantado en el instante exacto en que arrancaba `server.py`, la conexión síncrona tiraba una excepción sin capturar que mataba `on_startup` y con él todo `aiohttp` — el panel ni siquiera llegaba a servir la página de login. Ahora arranca igual y se conecta solo apenas Mosquitto esté disponible.
