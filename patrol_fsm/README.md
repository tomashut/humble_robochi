...
El nodo guarda su estado (`state`, `current_waypoint`, `fail_count`, `round_id`, `actividad_previa`, `saved_at`) en un JSON (`state_file`, default `~/.local/share/patrol_fsm/state.json`) cada vez que algo de eso cambia, con escritura atómica (archivo temporal + rename): protege contra que un crash del propio proceso (excepción, `kill -9`) deje `state.json` a medio escribir — el archivo real nunca se toca hasta que el temporal está completo.

**`fsync` antes del rename (2026-08-16).** El truco de archivo-temporal-y-rename por sí solo no garantiza nada contra un corte de luz real (a diferencia de un crash del proceso, ahí el sistema operativo entero se va sin avisar): el contenido puede quedar solo en el buffer del sistema operativo, no físicamente en el disco, en el instante justo del corte, aunque Python ya haya "terminado" de escribir. Ahora, antes de hacer el rename, se llama a `f.flush()` + `os.fsync(f.fileno())`, que fuerza a que el contenido esté realmente en disco antes de seguir — bloquea hasta tener esa confirmación, en vez de asumir que ya está. Relevante en concreto para este proyecto: el hardware objetivo es una Raspberry Pi con tarjeta SD, y las SD son conocidas por tener su propio cacheo de escritura interno que puede demorar (o directamente fallar en confirmar) escrituras — sin `fsync`, no hay ninguna garantía propia del código contra eso, solo la esperanza de que el sistema de archivos se porte bien. No se agregó el `fsync` del directorio contenedor (para que el rename en sí también quede garantizado) — cubre un caso todavía más marginal, se dejó afuera a propósito por relación costo/beneficio.
