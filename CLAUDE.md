...
- **Frontera de propiedad:** ROS 2 + Nav2 + slam_toolbox + AMCL + ros2_control se usan tal cual (open source de grado industrial, NO reescribir su lógica interna — sí está bien configurarlos o extenderlos por sus puntos de extensión oficiales: behavior trees, plugins, parámetros). El desarrollo propio se concentra en la capa de aplicación de seguridad. Nota: AMCL/slam_toolbox son la elección correcta para la etapa actual (interior/simulación); para el rover de exteriores es esperable que la localización cambie (GPS RTK, `robot_localization`, etc.) — no asumir que esa pieza puntual sobrevive sin cambios.
