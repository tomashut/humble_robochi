...
- **`.../events`** (publica, **sin retener**, QoS 1, agregado 2026-08-12) — repropaga el tópico ROS `patrol_events` tal cual, un mensaje por evento del "libro de rondas digital" de `patrol_fsm` (ver `patrol_fsm/README.md`, sección "Registro de ejecuciones"). Sin retener, a propósito: es un hecho puntual ("esto pasó a tal hora"), no un estado — un cliente que se conecta más tarde no debe recibir la última alarma como si acabara de pasar. Este es el canal pensado para que algo del otro lado (panel/central) construya semántica de alarma (pendiente-hasta-reconocida) separada de `.../state`, que solo contesta "cómo está el robot ahora". `comms_agent` no interpreta el contenido — es un reenvío tonto, la misma lógica que ya se aplica a `.../state`/`.../position`. **Caveat de QoS, sin resolver todavía:** un corte de `comms_agent` hacia Mosquitto puede perder eventos que se intenten publicar mientras está desconectado — a diferencia de `.../cmd` (ver "Sesión persistente" más abajo), la sesión persistente de Mosquitto no ayuda acá: `clean_session=False` retiene mensajes para lo que `comms_agent` está **suscripto** (`.../cmd`), no lo que **publica** (`.../events`); un evento que se intenta mandar durante el corte solo sobrevive si la cola local de `paho-mqtt` lo retiene hasta el reconnect, y esa cola se pierde si el proceso se reinicia antes de reconectar. A diferencia de `state`/`position`, que se autocorrigen al reconectar (ver el punto siguiente), un evento perdido así está perdido para siempre por este canal. El JSONL en el robot sigue siendo el registro autoritativo; MQTT es el canal en tiempo real, no la fuente de verdad.
...
**Sesión persistente para no perder comandos durante un corte (2026-08-14).** `__init__` ahora usa `mqtt.Client(client_id=f'comms_agent-{installation_id}-{robot_id}', clean_session=False)` en vez de `mqtt.Client()` (sin `client_id`, `clean_session=True` por default de la librería). Antes, un comando (`pause`, por ejemplo) mandado por MQTT mientras `comms_agent` estaba desconectado de Mosquitto se perdía para siempre, aunque el enlace volviera segundos después — con `clean_session=True`, Mosquitto no tiene de quién acordarse entre conexiones, así que no encola nada para nadie. Con `clean_session=False` y un `client_id` fijo (necesario: Mosquitto reconoce sesiones por `client_id`, y un `client_id` auto-generado cambiaría en cada reconexión), Mosquitto retiene los mensajes QoS 1 del tópico `.../cmd` mientras `comms_agent` está desconectado, y se los entrega apenas reconecta. Verificado en vivo: `comms_agent` caído, se manda `mosquitto_pub -q 1` con un comando, se vuelve a levantar `comms_agent` — el comando aparece procesado solo, sin mandar nada nuevo. Los demás tópicos (`state`/`position`/`heartbeat`/`events`) no dependen de esto — `state`/`position` se autocorrigen al reconectar (ver el punto de resync más arriba), y `heartbeat`/`events` son a propósito volátiles (un heartbeat o evento viejo reentregado tarde es ruido, no información).
